...
3. Pass your password and `salt` into `pbkdf2` (with 100,000+ iterations) to turn your text password into a 32-byte secret key. // Will use Argon2 instead
...
5. Re-run `pbkdf2` using the user-provided password and the extracted `salt` to recreate the exact 32-byte key. // Argon2 instead of pbkdf2
